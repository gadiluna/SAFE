...
class modelSAFE:
    ...
    def __init__(self, flags, embedding_matrix):
        if flags is not None and embedding_matrix is not None:
    ...
    def embedd(self, nodi_input, lengths_input):
        ...
        out_embedding = self.session.run(self.network.function_1, feed_dict={
            self.network.x_1: nodi_input,
            self.network.len_1: lengths_input})
